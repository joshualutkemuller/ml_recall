...
- `ml_recall.modeling.artifacts` persists fitted bundles as validated JSON artifacts with schema, version, horizon, feature, explicit reproducibility metadata, and a canonical SHA-256 digest for reproducible promotion.
- `ml_recall.scoring.pipeline.RecallScorer` loads a persisted bundle, validates scoring rows, filters requested horizons, assigns risk bands, and emits contribution-based reason codes.
